...
library(class)
#讀資料
dataOringin <- read.table("D:/Shared/M04A-mon.csv",header = FALSE ,sep = ",")
#給欄位名稱
...
statdf <- data.frame(station)
names(statdf) <- "start"

#篩選變數end開頭為01 與 type為小客車
data1 <- local({
  dataOringin[grep("^01",dataOringin$end),] %>%
    filter(type=="31") 
})

#把資料與路段合併篩出 想要的路段資料 並篩選時間
inn <- inner_join(data1,statdf)
...
#pivot資料 把資料整理成模型可用的
datetime <- dcast(x,date~dateT ,value.var = "time")
datenumber <- dcast(x,date~dateT,value.var = "number")
finalT <- left_join(x=datetime,y=datenumber,by="date")
